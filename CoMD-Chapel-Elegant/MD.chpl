...
local {
      var keTemp = 0.0;
      var peTemp = 0.0;
      forall (box, pe) in zip(MyDom.cells[MyDom.localDom], Sim.pe[Sim.pe.localSubdomain()]) with (+ reduce keTemp, + reduce peTemp) {
        for i in 1..box.count {
          const ref atom = box.atoms[i];
...
tArray[timerEnum.FORCE].start();
  if(replicateForce) then globalForce.computeLocal(); else globalForce.compute();
...
tArray[timerEnum.INITGRID].start();
  ...
  if(lat < 0.0) then latticeConstant = globalForce.lat;


  var targetLocales = defineTargetLocales();
  var Space = defineSpace(latticeConstant);
  Sim = new Simulation(targetLocales, Space);

if useChplVis then tagVdebug("initGrid");
  initGrid(latticeConstant, globalForce);
if useChplVis then pauseVdebug();
tArray[timerEnum.INITGRID].stop();

tArray[timerEnum.EPILOGUE].start();
  globalForce.epilogue();
...
if useChplVis then pauseVdebug();
  ...
  const cutoff = globalForce.cutoff;
...
tArray[timerEnum.F1].stop();
  ...
  Sim.vSim.eInit = keTotal + peTotal;
  ...
  writef("Initial energy : %14.12dr, atom count : %i \n", Sim.vSim.eInit/Sim.vSim.nAtomsInit, Sim.vSim.nAtomsInit);
...
if useChplVis then tagVdebug("advanceVelocity");
  coforall ijk in locDom {
    on locGrid[ijk] {
      ...
      const MyDom = Grid[ijk];
local {
      forall (box, f) in zip(MyDom.cells[MyDom.localDom], Sim.f[Sim.f.localSubdomain()]) {
        for i in 1..box.count {
          ref a = box.atoms(i);
          a.v += ((dt/a.mass)*f(i));
        }
...
tArray[timerEnum.TOTAL].stop();
  ...
  const eInitial = Sim.vSim.eInit/Sim.vSim.nAtomsInit;
  ...
  const nAtomsDelta = numAtoms - Sim.vSim.nAtomsInit;
