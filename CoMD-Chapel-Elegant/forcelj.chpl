...
if useChplVis then tagVdebug("computeLJForce");
    ...
    coforall ijk in locDom {
      on locGrid[ijk] {
        ...
        coforall (box, f, pe, boxIdx) in zip(MyDom.cells[MyDom.localDom], Sim.f[Sim.f.localSubdomain()], Sim.pe[Sim.pe.localSubdomain()], MyDom.localDom) {
...
local {
        const neighs = {-1..1, -1..1, -1..1};
        coforall (box, f, pe, boxIdx) in zip(MyDom.cells[MyDom.localDom], Sim.f, Sim.pe, MyDom.localDom) {
          for i in 1..box.count {
            f(i)  = (0.0, 0.0, 0.0);
            pe(i) = 0.0;
          }
