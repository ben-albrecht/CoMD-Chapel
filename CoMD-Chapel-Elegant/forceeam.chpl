...
if useChplVis then tagVdebug("computeEAMForce");
    ...
    coforall ijk in locDom {
      on locGrid[ijk] {
        ...
        coforall (box, f, pe, dfEmbed, rhoBar, boxIdx) in zip(MyDom.cells[MyDom.localDom], Sim.f, Sim.pe, MyEAMDom.dfEmbed[MyDom.localDom], MyEAMDom.rhoBar[MyDom.localDom], MyDom.localDom) {
...
if useChplVis then tagVdebug("computeEAMForce");
    coforall ijk in locDom {
      on locGrid[ijk] {
        ...
        const force = MyDom.force : ForceEAM;
        const neighs = {-1..1, -1..1, -1..1};
        coforall (box, f, dfEmbed, boxIdx) in zip(MyDom.cells[MyDom.localDom], Sim.f, MyEAMDom.dfEmbed[MyDom.localDom], MyDom.localDom) {
          for n in neighs {
            const ref nBox = MyDom.cells[boxIdx + n];
